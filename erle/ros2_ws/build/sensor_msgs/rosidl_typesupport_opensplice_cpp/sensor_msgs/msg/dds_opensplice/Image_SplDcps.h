#ifndef IMAGE_SPLTYPES_H
#define IMAGE_SPLTYPES_H

#include "ccpp_Image_.h"

#include <c_base.h>
#include <c_misc.h>
#include <c_sync.h>
#include <c_collection.h>
#include <c_field.h>

#include "Header_SplDcps.h"

extern c_metaObject __Image__sensor_msgs__load (c_base base);

extern c_metaObject __Image__sensor_msgs_msg__load (c_base base);

extern c_metaObject __Image__sensor_msgs_msg_dds___load (c_base base);

extern c_metaObject __sensor_msgs_msg_dds__Image___load (c_base base);
extern const char * __sensor_msgs_msg_dds__Image___keys (void);
extern const char * __sensor_msgs_msg_dds__Image___name (void);
struct _sensor_msgs_msg_dds__Image_ ;
extern  c_bool __sensor_msgs_msg_dds__Image___copyIn(c_base base, struct sensor_msgs::msg::dds_::Image_ *from, struct _sensor_msgs_msg_dds__Image_ *to);
extern  void __sensor_msgs_msg_dds__Image___copyOut(void *_from, void *_to);
struct _sensor_msgs_msg_dds__Image_ {
    struct _std_msgs_msg_dds__Header_ header_;
    c_ulong height_;
    c_ulong width_;
    c_string encoding_;
    c_octet is_bigendian_;
    c_ulong step_;
    c_sequence data_;
};

#endif
