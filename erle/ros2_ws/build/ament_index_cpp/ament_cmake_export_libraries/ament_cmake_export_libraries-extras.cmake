...
if(NOT "${_exported_library_targets} " STREQUAL " ")
  ...
  while(_i LESS _length)
    ...
    if("${_arg}" MATCHES "^debug|optimized|general$")
      ...
      math(EXPR _i "${_i} + 1")
      if(_i EQUAL _length)
        message(FATAL_ERROR "Package 'ament_index_cpp' passes the build configuration keyword '${_cfg}' as the last exported target")
      endif()
    ...
    find_library(
      ...
      PATHS "${ament_index_cpp_DIR}/../../../lib"
      NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH
    )

    if(NOT _lib)
      # warn about not existing library and ignore it
      message(FATAL_ERROR "Package 'ament_index_cpp' exports the library '${_library_target}' which couldn't be found")
    elseif(NOT IS_ABSOLUTE "${_lib}")
      # the found library must be an absolute path
      message(FATAL_ERROR "Package 'ament_index_cpp' found the library '${_library_target}' at '${_lib}' which is not an absolute path")
    elseif(NOT EXISTS "${_lib}")
      # the found library must exist
      message(FATAL_ERROR "Package 'ament_index_cpp' found the library '${_lib}' which doesn't exist")
    else()
      list(APPEND ament_index_cpp_LIBRARIES ${_cfg} "${_lib}")
    endif()
  endwhile()
endif()

# append absolute libraries to ament_index_cpp_LIBRARIES
if(NOT "${_exported_absolute_libraries} " STREQUAL " ")
  ...
  while(_i LESS _length)
    ...
    if(NOT EXISTS "${_absolute_library}")
      # the found library must exist
      message(WARNING "Package 'ament_index_cpp' exports the library '${_absolute_library}' which doesn't exist")
    else()
      list(APPEND ament_index_cpp_LIBRARIES ${_cfg} "${_absolute_library}")
    endif()
  endwhile()
endif()
  ...
  while(_i LESS _length)
    ...
    if(NOT _library_dirs)
      ...
      if(NOT _lib)
        ...
        message(WARNING "Package 'ament_index_cpp' exports library '${_library_name}' which couldn't be found")
      endif()
    else()
      ...
      if(NOT _lib)
        # warn about not existing library and later ignore it
        message(WARNING "Package 'ament_index_cpp' exports library '${_library_name}' with LIBRARY_DIRS '${_library_dirs}' which couldn't be found")
      endif()
    endif()
    if(_lib)
      list(APPEND ament_index_cpp_LIBRARIES ${_cfg} "${_lib}")
...
# deduplicate ament_index_cpp_LIBRARIES
